...
const CLASH_MOVE_IDS = [
  "starforged/moves/combat/clash",
  "classic/moves/combat/clash",
];

export interface ClashDialogProps {
  open: boolean;
  onClose: () => void;
}

export function ClashDialog({ open, onClose }: ClashDialogProps) {
  ...
  const move = useMemo(
    () => CLASH_MOVE_IDS.map((id) => moveMap[id]).find(Boolean),
    [moveMap]
  );
        ...
        {step === "setup" && (
          <Box>
            <Typography variant="body2" color="text.secondary" mb={2}>
              When you trade blows with your foe, roll +iron to hold firm or
              +edge to give ground.
            </Typography>
            ...
            <TextField
              ...
              multiline
              minRows={2}
              placeholder="How do you meet their attack?"
              value={playerContext}
          ...
          <Box>
            ...
            {rollData.outcome === ROLL_RESULT.HIT && (
              <Alert severity="success" sx={{ mb: 2 }}>
                You drive them back. Mark progress <strong>twice</strong>. Your
                foe must Endure Harm.
              </Alert>
            )}

            {rollData.outcome === ROLL_RESULT.WEAK_HIT && (
              <Alert severity="warning" sx={{ mb: 2 }}>
                You trade blows. Mark progress <strong>once</strong>. You must
                also Endure Harm (−1 to −3 health based on your foe&apos;s
                ferocity).
              </Alert>
            )}
            ...
            {rollData.outcome === ROLL_RESULT.MISS && (
              <PayThePriceSection
                missText="You are outmatched. Endure Harm twice (your foe inflicts their full harm). Pay the Price."
            ...
            <TextField
              ...
              multiline
              minRows={2}
              placeholder="How does the exchange unfold?"
